...
async function signup(parent, args, context, info) {
  ...
  async function login(parent, args, context, info) {
    ...
    // You may notice a secret used while signing the tokens. 
    // That secret is very important—it verifies the identity of the token. 
    // A user may also pass a counterfeit token generated by themselves, 
    // but we are signing the token with this unique secret known only to us,
    // therefore, we can guarantee the authenticity of the token by using this secret.
    // This way users can authenticate themselves with a signed token.
    const token = jwt.sign({ userId: user.id }, APP_SECRET)
      ...
      return context.prisma.createProject({
            ...
            avi_roles_on_project: { 
                ...
                create:                
                {
                  ...
                  avi_cinematographer: args.avi_roles_on_project.cinematographer,
